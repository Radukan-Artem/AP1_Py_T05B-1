...
def login():
    if request.method == 'POST':
        ...
        if jwt_response.accessToken is not None and jwt_response.refreshToken is not None:
            return jsonify(message = "login successful", accessToken=jwt_response.accessToken, refreshToken = jwt_response.refreshToken), 200
        else:
            return jsonify({"error": "login failed"}), 401
        
    return render_template('login.html')

@auth.route('/sing_up', methods=['post',  'get'])
def sing_up():
    if request.method == 'POST':
        auth_service = current_app.config["container"].authorization_service
        username = request.json.get("login", None)
        ...
        if result_request:
            jwt_response = auth_service.authorization(jwt_request)
            return jsonify(message = "sign up successful", accessToken=jwt_response.accessToken, refreshToken = jwt_response.refreshToken), 200
        else:
...
def refresh_access_token(refresh_token):
    ...
    return jsonify(message = "successful", accessToken=jwt_response.accessToken, refreshToken = jwt_response.refreshToken), 200
